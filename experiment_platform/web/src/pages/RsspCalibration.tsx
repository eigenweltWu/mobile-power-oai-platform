...
import { api } from '../api';
import { DEFAULT_GNB_CONFIGURATION, GnbConfigurationFields, TABLE2_MCS_RANGE, type GnbConfigurationKey } from '../components/GnbConfigurationFields';
...
const DEFAULT_GNB: GnbConfiguration = {
  ...DEFAULT_GNB_CONFIGURATION, puschTargetMode: 'manual',
};
...
export default function RsspCalibration({ onBack }: { onBack: () => void }) {
  ...
  const [view, setView] = useState<'radio' | 'calibration' | 'records'>('radio');
  ...
  const setGnbField = (key: GnbConfigurationKey, value: string) => {
    if (key !== 'ulTrafficMbps') setGnbForm((current) => ({ ...current, [key]: key === 'schedulerMode' ? value : Number(value) } as GnbConfiguration));
  };

  return <div className="stack page-workspace calibration-workspace">
    ...
    <div className="tabs workspace-tabs" role="tablist" aria-label="Calibration stages"><button className={view === 'radio' ? 'active' : ''} onClick={() => setView('radio')}>1 · Radio setup</button><button className={view === 'calibration' ? 'active' : ''} onClick={() => setView('calibration')}>2 · Calibrate {status.running && <Badge tone="good">LIVE</Badge>}</button><button className={view === 'records' ? 'active' : ''} onClick={() => setView('records')}>3 · Records <Badge tone="muted">{status.records.length}</Badge></button></div>
    ...
    <Card title="gNB Startup Configuration" sub="Apply the complete OAI radio and uplink configuration with one verified gNB restart.">
      <div className="stack">
        <GnbConfigurationFields value={gnbForm} onChange={setGnbField} disabled={!!busy || status.running} supportedBandwidthMHz={gnb?.supportedBandwidthMHz} />
